...
#pragma GCC diagnostic pop // Restore previous warning settings.

// CPU spec struct
typedef struct CPUSpec {
    char Vendor[13];
    CPUVendor VendorID;
    uint32_t FeaturesEAX;
    uint32_t FeaturesEBX;
...
/// Feature is the specific feature flag to check
bool CPUHasFeature(uint32_t featureSet, uint32_t feature);
